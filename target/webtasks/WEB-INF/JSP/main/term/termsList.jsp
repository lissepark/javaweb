			...
			<tr>
				...
				<td style="padding-left: 50px">
                    <select id="opening_list" name="chooser">
                        <option disabled selected="selected" value="">Choose the term</option>
				...
				<td style="padding-left: 20px">
                    <input type="submit" value="Выбрать" id="button">
                </td>
			</tr>
            </table>
    </form>
			...
			<tr height="80px" style="font-size: large; font-weight: bold;">
				<td colspan="3">Длительность семестра: ${selectedTerm.duration}
                    <c:choose>
                        ...
                        <c:when test="${Math.abs(selectedTerm.duration % 10)==0}">
                            недель
                        </c:when>
                        <c:when test="${Math.abs(selectedTerm.duration % 10)>4&&
                        Math.abs(selectedTerm.duration % 10)<10}">
                            недель
                        </c:when>
                        <c:when test="${Math.abs(selectedTerm.duration % 10)==1}">
                            неделя
                        </c:when>
                        <c:otherwise>
                            недели
						...
						<table cellspacing="0">
							<form action="${CONTEXT }${CURRENT_MAPPING}/termCreating.php" method="GET">
								<tr>
									<td style="padding-bottom: 20px; padding-top: 0">
                                        <input type="submit" class="button_long_name" value="Создать семестр...">
                                    </td>
								</tr>
							</form>
							...
							<tr>
								<td style="padding-bottom: 20px"><input type="button"
									class="button_long_name" value="Удалить текущий семестр..." onclick="deleteTerm()"></td>
							</tr>
						</table>
					</c:if>
                </td>
